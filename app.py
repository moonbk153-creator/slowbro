...
def load_from_db():
    ...
    # [수정] 오차의 빈칸(NaN)을 문자로 채우지 않고 그대로 둡니다. (포맷팅에서 처리)
...
if not display_df.empty:
    ...
    # [핵심 수정] 숫자 값들을 모두 소수점 1자리("{:.1f}")로 강제 포맷팅합니다. 결측치(빈칸)는 "-"로 표시합니다.
    ...
    # 엑셀 다운로드용 데이터는 순수 숫자 형태 유지 (엑셀 내부에서 활용 가능하도록)
...
with st.expander("🛠️ 관리자 전용 메뉴 (데이터 수정/삭제 및 과거 엑셀 업로드)"):
    ...
    if input_password == ADMIN_PASSWORD:
        ...
        with tab1:
            ...
            with col_admin2:
                if action == "데이터 삭제":
                    st.warning("삭제된 데이터는 복구할 수 복구할 수 없습니다.")
