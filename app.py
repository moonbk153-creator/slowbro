...
with st.expander("🛠️ 관리자 전용 메뉴 (데이터 수정/삭제 및 관리)"):
    ...
    if input_password == ADMIN_PASSWORD:
        ...
        # [신규 추가] 탭 5개로 늘림: 장기 미생산 제품 조회 추가
        ...
        with tab5:
            ...
            if inactive_products:
                ...
                # 경과일이 가장 오래된 것부터 최상단으로 내림차순 정렬 (순서용 숫자 컬럼은 숨김 처리)
                df_inactive = df_inactive.sort_values(by="경과일수_순서용", ascending=False).drop(columns=["경과일수_순서용"])
