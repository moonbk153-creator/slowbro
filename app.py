...
def get_target_history_df(p_name):
    conn = sqlite3.connect(DB_FILE)
    df = pd.read_sql_query('SELECT effective_date, target_value FROM target_history WHERE product_name = ? ORDER BY effective_date DESC, id DESC', conn, params=(p_name,))
    conn.close()
    return df
...
def admin_menu_dialog():
    if st.text_input("🔒 비밀번호", type="password") == ADMIN_PASSWORD:
            ...
            if tdf.empty: st.info("기록 없음")
                ...
                if mode == "미확인 ❌":
                    ...
                    if st.button("✅ 선택 확인"):
                        ...
                        st.cache_data.clear()
                        st.session_state['show_toast'] = "확인 완료!"
                        st.rerun()
                else:
                    ...
                    if st.button("🔄 선택 복구"):
                        ...
                        st.cache_data.clear()
                        st.session_state['show_toast'] = "복구 완료!"
                        st.rerun()
        with t2:
            ...
            with col2:
                if act == "삭제" and st.button("🗑️ 삭제"):
                    delete_from_db(tid)
                    st.cache_data.clear()
                    st.session_state['show_toast'] = "삭제됨!"
                    st.rerun()
                elif act == "수정":
                    conn = sqlite3.connect(DB_FILE)
                    c = conn.cursor()
                    ...
                    if row:
                        ...
                        if st.button("✏️ 수정"):
                            ...
                            st.cache_data.clear()
                            st.session_state['show_toast'] = "수정됨!"
                            st.rerun()
        with t3:
        ...
        with t8:
            ...
            if st.button("➕ 추가") and add_worker(nw): st.cache_data.clear(); st.rerun()
            if CURRENT_WORKERS:
                dw = st.selectbox("기존 작업자", CURRENT_WORKERS)
                if st.button("➖ 삭제"): delete_worker(dw); st.cache_data.clear(); st.rerun()
...
# [UI 개선 4] 입력 동선 통합 (메인 화면 상단)
st.subheader("📝 데이터 등록")
tab_n, tab_q = st.tabs(["📋 일반 데이터 등록", "⚡ 진행 중인 라인 빠른 추가"])
with tab_n:
    with st.container(border=True):
        cs1, cs2, cs3, cs4 = st.columns(4)
        with cs1: p_date = st.date_input("생산일").strftime("%Y-%m-%d")
        with cs2: 
            eq = st.selectbox("생산 설비", EQUIPMENT_LIST)
            eq_cl = eq.lower().replace(" ","")
        with cs3: wk = st.selectbox("작업자", CURRENT_WORKERS)
        with cs4:
            amt = st.selectbox("투입량", ["1.35kg","2.5kg","3.75kg"]) if "버닝" in eq_cl else ("12kg" if "태환" in eq_cl else "25kg" if "프로밧" in eq_cl else "60kg" if "60" in eq_cl else "120kg" if "120" in eq_cl else "-")
            if "버닝" not in eq_cl: st.text_input("투입량 (고정)", amt, disabled=True)
        st.markdown("---")
        cs5, cs6, cs7 = st.columns([2,1,1])
        with cs5: pd_name = st.selectbox("🔍 제품명", list(TARGET_DATA.keys()))
        tgt_val = get_historical_target(pd_name, p_date)
        with cs6: st.text_input("기준 색도", f"{float(tgt_val):.1f}", disabled=True)
        with cs7: meas_val = st.number_input("측정 색도", value=float(tgt_val), step=0.1)
        
        if ACTIVE_NOTICES.get(pd_name): st.warning(f"📢 **전달사항** {ACTIVE_NOTICES[pd_name]}")
        
        cs8, cs9 = st.columns([3,1])
        with cs8: rmk = st.text_input("특이사항")
        with cs9:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("등록하기", type="primary", use_container_width=True):
                if check_recent_duplicate(p_date, eq, pd_name, meas_val): st.error("⚠️ 중복 데이터 방지")
                else:
                    diff = round(meas_val - tgt_val, 1)
                    save_to_db(p_date, eq, wk, pd_name, tgt_val, meas_val, diff, "합격 🟢" if abs(diff)<=2.0 else "불합격 🔴", rmk, amt)
                    st.cache_data.clear()
                    st.session_state['show_toast'] = "정상 등록됨!"
                    st.rerun()
                ...
                with cq4:
                    ...
                    if st.button("🚀 빠른 등록", type="primary", use_container_width=True):
                        if check_recent_duplicate(today_str_kst, qe, qp, qm): st.error("⚠️ 중복")
                            ...
                            st.cache_data.clear()
                            st.session_state['show_toast'] = "빠른 등록 완료!"
                            st.rerun()
