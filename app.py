...
# ==========================================
# 2. 엑셀 기준표 및 공지사항 로드 & 이력 동기화
# ==========================================
@st.cache_data
def load_target_data():
    try:
        # C열(제품명), D열(기준색도), E열(공지사항)을 로드합니다.
        df = pd.read_excel(EXCEL_FILE, usecols="C:E", header=1)
        ...
        for index, row in df.iterrows():
            ...
            # E열에 공지사항 데이터가 기입되어 있으면 저장
            if len(row) > 2 and not pd.isna(row.iloc[2]):
                notice_dict[prod] = str(row.iloc[2]).strip()
            else:
                notice_dict[prod] = ""
                
        return target_dict, notice_dict
    except FileNotFoundError:
        st.error(f"⚠️ '{EXCEL_FILE}' 파일을 찾을 수 없습니다.")
        return {"(엑셀 파일 없음)": 0.0}, {}

init_db()
TARGET_DATA, NOTICE_DATA = load_target_data()
...
# ---------------------------------------------------------
# [신규 기능] 제품 선택 시 엑셀 E열에 전달사항이 적혀 있으면 팝업 알림 수행
# ---------------------------------------------------------
if NOTICE_DATA.get(selected_product):
    st.sidebar.warning(f"📢 **[작업자 전달사항]**\n\n{NOTICE_DATA[selected_product]}", icon="🚨")
...
with st.expander("🛠️ 관리자 전용 메뉴 (데이터 수정/삭제 및 과거 엑셀 업로드)"):
    ...
    if input_password == ADMIN_PASSWORD:
        ...
        # 순수 DB 파일을 파일 형태로 직접 백업 추출할 수 있는 다운로드 단추
        try:
