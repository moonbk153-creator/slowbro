...
# 오타 및 대소문자 무관하게 강제 일칭 매핑 정렬 함수 생성 (안전장치)
...
if not display_df.empty:
    ...
    # [수정 반영] 요청하신 대로 정렬은 '오늘'과 '특정 일자 지정'인 당일 화면에서만 엄격하게 적용됩니다.
    if date_filter_mode == "오늘(Today)":
        display_df = display_df[display_df['생산일'] == today_str_kst]
        # 설비 정렬 순번 생성 후 2단계 순차 정렬 (설비 순서 -> 내부 생산 순서)
        ...
        # '전체 기간' 선택 시에는 설비별 정렬을 거치지 않고, 기존 데이터베이스의 역사적 흐름(최신 생산일순)을 원본 유지합니다.
...
# [소문자 kg 통일 및 글자색 검정 통일] 표 배경 색상 맵핑 함수
def highlight_equipment(s):
    ...
    for val in s:
        ...
        if '버닝' in val_clean:
            ...
            colors.append('background-color: #D4EFDF; color: black; font-weight: bold;') # 파스텔톤 진녹색
        else:
            colors.append('')
    return colors
