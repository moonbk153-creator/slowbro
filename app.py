...
# [완벽 분석 조치] 날짜 강제 정규화 함수 (엑셀 찌꺼기 완벽 차단)
...
def init_db():
    ...
    # [완벽 분석 조치] 컬럼 누락으로 인한 DB Crash를 100% 방지하는 안전한 컬럼 추가 기법
    ...
    if "input_amount" not in existing_columns:
...
def save_to_db(prod_date, equipment, worker, product, target, measured, diff, status, remarks, input_amount="-"):
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (timestamp, prod_date, equipment, str(worker).strip(), product, target, measured, diff, status, remarks, input_amount))
    conn.commit()
    conn.close()
...
# [완벽 분석 조치] 0건 출력 현상을 해결하기 위해 쿼리 구조를 완전히 안전하게 재설계했습니다.
def load_from_db():
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    
    # DB에 컬럼이 있는지 없는지 파악하여 에러를 원천 차단
    cursor.execute("PRAGMA table_info(color_records)")
    columns = [info[1] for info in cursor.fetchall()]
    
    input_amt_col = "c.input_amount" if "input_amount" in columns else "'-'"
    checked_col = "c.checked" if "checked" in columns else "0"
    remarks_col = "c.remarks" if "remarks" in columns else "''"
    ...
    query = f"""
        SELECT 
            ...
            c.equipment as 생산설비, 
            COALESCE({input_amt_col}, '-') as 투입량,
            c.worker as 작업자, 
            c.product_name as 제품명, 
            c.measured_value as 측정색도, 
            COALESCE({remarks_col}, '') as 특이사항,
            COALESCE({checked_col}, 0) as checked_status,
            COALESCE(
...
def update_db(record_id, new_date, new_equip, new_worker, new_product, new_target, new_measured, new_diff, new_status, new_remarks, new_input_amount, new_checked=0):
    ...
    if "checked" in columns:
        cursor.execute('''
            ...
            WHERE id=?
        ''', (new_date, new_equip, str(new_worker).strip(), new_product, new_target, new_measured, new_diff, new_status, new_remarks, new_input_amount, new_checked, record_id))
    else:
        cursor.execute('''
            UPDATE color_records 
            SET production_date=?, equipment=?, worker=?, product_name=?, target_value=?, measured_value=?, difference=?, status=?, remarks=?, input_amount=?
            WHERE id=?
        ''', (new_date, new_equip, str(new_worker).strip(), new_product, new_target, new_measured, new_diff, new_status, new_remarks, new_input_amount, record_id))
...
if date_filter_mode == "오늘(Today)":
    ...
    metric_title = f"전체 기간 누적" if not search_query else f"'{search_query}' 검색"
...
if not display_df.empty:
    # 메인 화면에서는 '확인여부' 열을 숨김 처리하여 쾌적하게 유지
    ...
    if date_filter_mode == "오늘(Today)":
        ...
        recent_batches = main_display_df[['제품명', '생산설비', '투입량', '작업자']].drop_duplicates().reset_index(drop=True)
        ...
        with col_q1:
            selected_batch_str = st.selectbox("이어서 측정할 제품을 선택하세요", batch_options, key="quick_batch_select")
...
with st.expander("🛠️ 관리자 전용 메뉴 (데이터 수정/삭제 및 관리)"):
    ...
    if input_password == ADMIN_PASSWORD:
        ...
        with tab1:
            st.write("🔍 **금일 생산 제품 확인 및 미확인 내역 전용 분리 관리**")
            ...
            if today_batches_df.empty:
                ...
                view_mode = st.radio("분리 조회 선택", ["미확인 내역만 전용 보기", "확인 완료 내역 보기"], horizontal=True, key="admin_view_mode")
                
                if view_mode == "미확인 내역만 전용 보기":
                    unverified_df = today_batches_df[today_batches_df['확인여부'] == "미확인 ❌"]
                    if unverified_df.empty:
                        st.success("🎉 금일 가동된 모든 배치의 검토 및 확인 완료 처리가 끝났습니다!")
                    else:
                        st.dataframe(unverified_df, use_container_width=True, hide_index=True)
                        st.markdown("<br>", unsafe_allow_html=True)
                        
                        to_confirm = st.multiselect(
                            "검토 및 확인 완료 처리할 배치를 선택하세요 (복수 선택 가능)", 
                            options=unverified_df['고유번호'].tolist(),
                            format_func=lambda x: f"고유번호 {x} - {unverified_df[unverified_df['고유번호']==x]['제품명'].values[0]} ({unverified_df[unverified_df['고유번호']==x]['생산설비'].values[0]} / {unverified_df[unverified_df['고유번호']==x]['작업자'].values[0]})"
                        )
                        
                        if st.button("✅ 선택한 배치 확인 처리하기", type="primary", use_container_width=True):
                            if not to_confirm:
                                st.warning("확인 처리할 배치를 선택해 주세요.")
                            else:
                                update_checked_status(to_confirm, 1)
                                st.cache_data.clear()
                                st.success(f"성공적으로 {len(to_confirm)}건의 생산 내역 확인 완료 처리를 끝냈습니다!")
                                st.rerun()
                                
                elif view_mode == "확인 완료 내역 보기":
                    verified_df = today_batches_df[today_batches_df['확인여부'] == "확인완료 ✅"]
                    if verified_df.empty:
                        st.info("💡 금일 확인 완료된 내역이 없습니다.")
                    else:
                        st.dataframe(verified_df, use_container_width=True, hide_index=True)
                        st.markdown("<br>", unsafe_allow_html=True)
                        
                        to_unconfirm = st.multiselect(
                            "미확인 상태로 되돌릴 배치를 선택하세요 (오선택 조치)", 
                            options=verified_df['고유번호'].tolist(),
                            format_func=lambda x: f"고유번호 {x} - {verified_df[verified_df['고유번호']==x]['제품명'].values[0]} ({verified_df[verified_df['고유번호']==x]['생산설비'].values[0]})"
                        )
                        
                        if st.button("🔄 선택한 배치 미확인 상태로 되돌리기", use_container_width=True):
                            if not to_unconfirm:
                                st.warning("되돌릴 배치를 선택해 주세요.")
                            else:
                                update_checked_status(to_unconfirm, 0)
                                st.cache_data.clear()
                                st.success(f"성공적으로 {len(to_unconfirm)}건의 배치를 미확인 상태로 되돌렸습니다.")
                                st.rerun()
        ...
        with tab2:
            ...
            with col_admin2:
                if action == "데이터 삭제":
                    ...
                    cursor.execute("PRAGMA table_info(color_records)")
                    columns = [info[1] for info in cursor.fetchall()]
                    chk_col = "checked" if "checked" in columns else "0"
                        ...
                        if all(col in df_upload.columns for col in required_cols):
                            ...
                            for index, row in df_upload.iterrows():
                                ...
                                cursor.execute('''
                                    INSERT INTO color_records (timestamp, production_date, equipment, worker, product_name, target_value, measured_value, difference, status, remarks, input_amount)
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                                ''', (current_time, prod_date_str, equip, str(worker).strip(), product, target, measured, diff, status, upload_remarks, upload_amt))
                                conn.commit()
                                conn.close()
                    ...
                    try:
                        ...
                        if all(col in df_history.columns for col in req_hist_cols):
                            ...
                            for index, row in df_history.iterrows():
                                product = str(row['제품명']).strip()
                                eff_date_str = safe_date_parse(row['적용시작일'])
                                if not eff_date_str:
