...
# ==============================================================================
# 💡 [핵심 정렬 및 정규화 엔진] 모든 공백과 날짜 찌꺼기를 세탁하고 재정렬합니다.
# ==============================================================================
...
def load_from_db():
    ...
    # 2. 강력한 재정렬: 최신 날짜가 무조건 위로 오도록 멱살 잡고 정렬 (오류 원천차단)
    ...
    # 5. [설비 첫 배치 🚀] 태그 계산 (시간 역순 정렬이므로 각 그룹의 제일 '마지막' 행이 역사상 최초 배치)
    ...
    # 6. [마지막 배치 🏁] 태그 계산 (시간 역순 정렬이므로 각 날짜의 제일 '첫' 행이 그날의 마지막 배치)
        ...
        with tab2:
            ...
            with col_m1:
                if st.button("🚀 전체 과거 데이터 'kg' 일괄 변환 실행"):
                    ...
                    st.rerun()
            with col_m2:
                # [강력해진 정화 기능] 작업자뿐만 아니라 설비와 제품명의 공백까지 모두 청소합니다.
                if st.button("🧹 DB 전체 텍스트 공백(띄어쓰기) 영구 정화"):
                    conn = sqlite3.connect(DB_FILE)
                    cursor = conn.cursor()
                    ...
                    conn.close()
                    st.cache_data.clear()
                    st.success(f"🎉 총 {updated_rows}건의 보이지 않는 공백 찌꺼기가 성공적으로 치료되었습니다!")
                    st.rerun()
