...
# [보안] Streamlit Secrets에서 암호를 불러옵니다. (관리자 비밀번호: 0322 / 접속 비밀번호: 27000833)
...
# [수정 완료] 전체기간 조회 & 검색어 없을 때는 총합계 카드를 완전히 숨김
if not (date_filter_mode == "전체 기간" and not search_query):
    ...
    # [수정 완료] 합격/불합격 카드를 제외하고 총 배치 수 1개만 직관적으로 표시
...
if not display_df.empty:
    ...
    # [스타일 유지] 특이사항은 노란색, 제품명은 굵게 하이라이트 지정
    ...
    st.dataframe(
        ...
        hide_index=True,
        # [스타일 유지] 글자가 가려지지 않게 특이사항/제품명의 최소 칸 너비를 큼직하게 강제 고정
        column_config={
