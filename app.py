...
# [핵심 보완] 모든 날짜 데이터를 YYYY-MM-DD로 강제 변환하는 철벽 정규화 함수
...
def init_db():
    ...
    try: cursor.execute("ALTER TABLE color_records ADD COLUMN remarks TEXT DEFAULT ''")
    ...
    try: cursor.execute("ALTER TABLE color_records ADD COLUMN input_amount TEXT DEFAULT '-'")
    except sqlite3.OperationalError: pass
    ...
    try: cursor.execute("ALTER TABLE color_records ADD COLUMN checked INTEGER DEFAULT 0")
    except sqlite3.OperationalError: pass
...
def load_from_db():
    ...
    # [에러 해결 1] 모든 날짜를 완벽한 YYYY-MM-DD로 포맷 정규화 (엑셀 찌꺼기 완벽 차단)
...
# [에러 해결 2] 검색창에서 실수로 공백만 입력했을 경우 발생하는 0건 오류 방지
with col_filter1:
...
if not display_df.empty:
    ...
    # [에러 해결 3] 날짜 문자열(==) 완벽 일치 비교로 0건 표시되는 문제 완벽 회피
...
# [에러 해결 4] 전체 기간일지라도 항상 메트릭(총 배치수)이 표시되도록 구조 수정
        ...
        with tab2:
            ...
            with col_admin2:
                if action == "데이터 삭제":
                    ...
                    conn = sqlite3.connect(DB_FILE)
                    c = conn.cursor()
                    c.execute("SELECT product_name, target_value, production_date, equipment, worker, measured_value, remarks, input_amount, COALESCE(checked, 0) FROM color_records WHERE id=?", (target_id,))
                    row = c.fetchone()
                    conn.close()
