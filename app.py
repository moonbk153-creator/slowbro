...
def load_from_db():
    ...
    # [핵심 수정: 날짜 오류 100% 방지] 엑셀 시간(00:00:00), 띄어쓰기, 슬래시(/) 등 모든 날짜 오류를 강제로 YYYY-MM-DD로 포맷 정규화
    parsed_dates = pd.to_datetime(df['생산일'], errors='coerce').dt.strftime('%Y-%m-%d')
    df['생산일'] = parsed_dates.fillna(df['생산일'].astype(str).str.strip().str[:10]).str.replace("/", "-")

    # 작업자 및 설비 띄어쓰기(공백) 오류 방지
...
# [수정] filter_date_str 변수를 조건문 밖에서 안전하게 빈 값으로 먼저 할당하여 NameError 원천 차단
filter_date_str = ""
with col_filter3:
...
if not display_df.empty:
    # 메인 화면에서는 확인여부 열을 숨김 처리하여 화면 쾌적하게 유지
    ...
    if input_password == ADMIN_PASSWORD:
        ...
        with tab1:
            ...
            if today_batches_df.empty:
                ...
                view_mode = st.radio("분리 조회 선택", ["미확인 내역만 전용 보기", "확인 완료 내역 보기"], horizontal=True)
