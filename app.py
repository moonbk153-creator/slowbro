...
# 2. 엑셀 기준표 로드 및 초기화
# ==========================================
init_db() # 데이터베이스가 초기화되면서 작업자 테이블도 함께 준비됩니다.
    ...
    if input_password == ADMIN_PASSWORD:
            ...
            with col_admin2:
                if action == "데이터 삭제":
                    ...
                    if row:
                        ...
                        # [동적 연동] DB에 저장된 작업자 리스트를 활용하되 과거 퇴사자 이름 처리
        ...
        with tab6:
            ...
            if not history_df.empty:
                ...
                # [동적 연동] DB에 저장된 작업자 순서대로 정렬하되, 퇴사자 이름이 있으면 맨 아래로
                ...
                styled_stats = stats_df.style.format({
                    ...
                    subset=['불량률(%)'], cmap='Reds'
                ).background_gradient(
                    subset=['평균 오차(절대값)'], cmap='Oranges'
                ).set_properties(
