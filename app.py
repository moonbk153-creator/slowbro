...
if not display_df.empty:
    ...
    if date_filter_mode == "오늘(Today)":
        ...
        # 1단계: 생산설비를 지정된 순서의 카테고리로 변경하여 우선 정렬 규칙 부여
        display_df['생산설비'] = pd.Categorical(display_df['생산설비'], categories=equipment_order, ordered=True)
        # 2단계: 설비순(오름차순) -> 설비 내에서는 생산 시간 순서(고유번호 오름차순)로 정렬
        display_df = display_df.sort_values(by=['생산설비', '고유번호'], ascending=[True, True])
        ...
        # 특정 일자 지정 화면도 오늘과 동일하게 구조화된 설비 순회 정렬 적용
        display_df['생산설비'] = pd.Categorical(display_df['생산설비'], categories=equipment_order, ordered=True)
        display_df = display_df.sort_values(by=['생산설비', '고유번호'], ascending=[True, True])
...
# [수정 기능] 전체 글자 색상 검정색 통일 및 부드러운 연한 파스텔 색상 맵핑
...
if not display_df.empty:
    styled_df = display_df.style.format(
        ...
        subset=['특이사항'], 
        # [수정 기능] 특이사항도 글자 색상 검정색(black)으로 지정
