...
if not display_df.empty:
    ...
    # [핵심 수정] 대소문자/띄어쓰기 무시를 위한 클렌징용 임시 정렬 컬럼 생성
    display_df['정렬용_설비'] = display_df['생산설비'].astype(str).str.upper().str.replace(" ", "")
    # "태환 12kg" -> "태환12kg" / "뷸러 60kg" -> "뷸러60kg" 형태로 완벽하게 표준화됨
    equipment_order_clean = ["버닝", "태환12kg", "프로밧25kg", "뷸러60kg", "뷸러120kg"]
    
    if date_filter_mode == "오늘(Today)":
        display_df = display_df[display_df['생산일'] == today_str_kst]
        display_df['정렬용_설비'] = pd.Categorical(display_df['정렬용_설비'], categories=equipment_order_clean, ordered=True)
        display_df = display_df.sort_values(by=['정렬용_설비', '고유번호'], ascending=[True, True])
    ...
    # 화면 출력 전 임시 컬럼 깔끔하게 삭제
    display_df = display_df.drop(columns=['정렬용_설비'])
...
# [수정] 대소문자/띄어쓰기 무시하고 완벽하게 색상을 맵핑하는 함수
def highlight_equipment(s):
    colors = []
    for val in s:
        # 띄어쓰기 제거 및 대문자 강제 변환
        val_clean = str(val).upper().replace(" ", "")
        
        if '버닝' in val_clean:
            colors.append('background-color: #E1F5FE; color: black; font-weight: bold;') # 하늘색
        elif '태환' in val_clean:
            colors.append('background-color: #FFF3CD; color: black; font-weight: bold;') # 노랑색
        elif '프로밧' in val_clean:
            colors.append('background-color: #FCE4EC; color: black; font-weight: bold;') # 핑크색
        elif '뷸러60' in val_clean:
            colors.append('background-color: #E8F5E9; color: black; font-weight: bold;') # 연녹색
        elif '뷸러120' in val_clean:
            colors.append('background-color: #D4EFDF; color: black; font-weight: bold;') # 진녹색
        else:
            colors.append('')
    return colors
